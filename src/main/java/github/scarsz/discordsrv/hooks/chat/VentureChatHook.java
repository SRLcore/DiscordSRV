...
public class VentureChatHook implements ChatHook {
    ...
    public void onVentureChat(VentureChatEvent event) {
        ...
        if (!DiscordUtil.strip(message).startsWith(prefix)) {
            ...
            DiscordSRV.debug(() ->
                    "A VentureChat message was received but it was not delivered to Discord because the message didn't start with \"" + prefix + "\" (DiscordChatChannelPrefixRequiredToProcessMessage): \"" +
                            finalMessage + "\"");
            return;
        }
        ...
        if (!DiscordSRV.config().getBoolean("Experiment_WebhookChatMessageDelivery")) {
            ...
            if (destinationChannel == null) {
                ...
                DiscordSRV.debug(() -> "Failed to find Discord channel to forward message from game channel " +
                        finalChannel);
                return;
            }
